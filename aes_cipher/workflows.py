"""
Application workflows that orchestrate the UI and AES-128 cipher logic.
"""
...
def _read_key() -> str:
    """Prompts for an AES-128 key (string/hex; validation to be added with implementation)."""
    while True:
        key = ui.prompt("Key (ví dụ 16 ký tự ASCII hoặc 32 hex): ").strip()
        if key:
            return key
...
def encrypt_flow():
    ...
    ui.clear()
    ui.banner()
    ui.boxed("ENCRYPT", "Nhập văn bản cần mã hóa và khóa AES-128. Chọn mode (ecb/cfb).")
